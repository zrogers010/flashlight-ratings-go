...
ALTER TABLE flashlight_specs
  ALTER COLUMN max_lumens DROP NOT NULL;
...
-- Step 2: Upsert reference tables
INSERT INTO use_cases (name, slug)
SELECT DISTINCT
  initcap(replace(tag_slug, '-', ' ')) AS name,
  tag_slug
FROM (
  SELECT lower(regexp_replace(trim(tag), '[^a-zA-Z0-9]+', '-', 'g')) AS tag_slug
  FROM cleaned c
...
-- Step 3: Upsert flashlights (separate statement so downstream can see them)
...
-- Step 4: Upsert specs (can now see all flashlights)
INSERT INTO flashlight_specs (
  ...
  recharge_type, battery_replaceable, weight_g, length_mm, head_diameter_mm,
  body_diameter_mm, switch_type, waterproof_rating, impact_resistance_m, body_material
)
SELECT
  f.id,
  c.max_lumens,
  ...
  CASE WHEN c.beam_pattern IN ('flood', 'throw', 'hybrid') THEN c.beam_pattern ELSE NULL END,
  CASE WHEN c.recharge_type IN ('usb-c', 'magnetic', 'none') THEN c.recharge_type ELSE NULL END,
  c.battery_replaceable,
  c.weight_g,
  c.length_mm,
  c.head_diameter_mm,
  c.body_diameter_mm,
  CASE WHEN c.switch_type IN ('tail', 'side', 'dual', 'rotary', 'twist') THEN c.switch_type ELSE NULL END,
  c.waterproof_rating,
...
-- Step 8: Amazon product snapshots
...
WHERE c.asin IS NOT NULL
  AND (c.amazon_rating_count IS NOT NULL OR c.amazon_average_rating IS NOT NULL
       OR c.current_price_usd IS NOT NULL OR c.image_url IS NOT NULL);

-- Step 9: Use case mapping
...
SELECT
  ...
  (SELECT count(*) FROM flashlights WHERE is_active) AS total_flashlights,
  (SELECT count(*) FROM flashlight_specs) AS total_specs,
  (SELECT count(*) FROM affiliate_links WHERE is_active) AS total_affiliate_links,
  (SELECT count(*) FROM flashlight_price_snapshots) AS total_prices;
